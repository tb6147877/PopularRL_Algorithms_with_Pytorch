...
class Critic(nn.Module, abc.ABC):
    ...
    def compute_gae(self, rewards:Tensor, values: Tensor, last_value:Tensor, gamma:float, lam:float)->tuple[Tensor, Tensor]:
        """Compute generalized advantage estimation (GAE) for a given episode.
                ...
                """
        horizon = len(rewards)
        advantages, returns = th.zeros_like(rewards, device=rewards.device), th.zeros_like(rewards, device=rewards.device)
        next_value = last_value
        #next_advantage = 0.0
        next_advantage = th.zeros((), device=rewards.device)
...
class CentralizedCritic(Critic):
    ...
    def value(self, state: Tensor, other_action: Tensor, n_actions: int = 6):
        ...
        one_hot = F.one_hot(masked_action, num_classes=n_actions).float().to(state.device)
    ...
    def update(self, dataset: dict):
        ...
        for epoch in range(self.epochs):
            ...
            for agent in dataset.keys():
                ...
                permutation = th.randperm(dataset_size, device=states.device)
        ...
        for agent, agent_trajectories in trajectories.items():
            for trajectory in agent_trajectories:
                ...
                with th.no_grad():
                    ...
                    last_value = th.zeros(1, device=values.device)
                    if len(states) > 0:
...
class Actor(nn.Module):
    ...
    def update(self, dataset:dict):
        ...
        for epoch in range(self.epochs):
            ...
            permutation = th.randperm(dataset_size, device=dataset["states"].device)
...
class Algorithm(nn.Module, abc.ABC):
    ...
    def __init__(self, env:AECEnv, batch_size:int=64, epochs:int=10, eps:float=0.2, c_ent:float=0):
        ...
        self.actors = nn.ModuleDict(
            {
                agent:Actor(statesize=self.ssize, n_actions=env.action_space(agent).n,batch_size=batch_size,eps=eps,epochs=epochs,c_ent=c_ent).to(self.device) for agent in env.agents
            }
        )
...
class MAPPO(Algorithm):
    ...
    def __init__(self, env: AECEnv, batch_size: int = 64, epochs: int = 10, eps: float = 0.2, c_ent: float = 0):
        ...
        self.centralized_critic = CentralizedCritic(self.ssize, batch_size, epochs).to(self.device)
    ...
    def split_trajectory(self, trajectory: list) -> dict[str, Tensor]:
        ...
        return {
            'states': th.as_tensor(np.array(states), dtype=th.float32, device=self.device),
            'rewards': th.as_tensor(rewards, dtype=th.float32, device=self.device),
            'actions': th.as_tensor(actions, dtype=th.int64, device=self.device),
            'other_actions': th.as_tensor(other_actions, dtype=th.int64, device=self.device),
            'logprobs': th.as_tensor(logprobs, dtype=th.float32, device=self.device),
        }
...
if __name__ == '__main__':
    ...
    algo = MAPPO(env, batch_size=1024, c_ent=0.01)
