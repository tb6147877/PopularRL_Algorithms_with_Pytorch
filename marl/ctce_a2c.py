...
class A2C(nn.Module):
    ...
    def compute_policy_loss(self, bs, br, bd, bns, logp_action_dict):
        with torch.no_grad():
            ...
            #advantage = predicted_value - br #use br as td target
    ...
    def compute_value_loss(self, bs, br, bd, bns, logp_action_dict):
        with torch.no_grad():
            td_value = self.target_value(bns).squeeze()
            td_value = br + self.gamma * td_value * (1 - bd)
        #td_value = br
...
def train(args, env, central_controller:A2C):
    policy_params = []
    ...
    for episode in range(args.num_episode):
        ...
        for i, agent in enumerate(env.agent_iter()):
            ...
            if env.agent_selection == "agent_0": # at this time, all agents finish their action and we can collect their state
                ...
                if done is True:
                    ...
                    value_loss = central_controller.compute_value_loss(bs, br, bd, bns, logp_action_dict)
                    value_optimizer.zero_grad()
                    value_loss.backward()
                    value_optimizer.step()

                    policy_loss = central_controller.compute_policy_loss(bs, br, bd, bns, logp_action_dict)
...
def eval(args):
    ...
    central_controller = A2C(num_agents = args.num_agents, num_states = args.num_states, num_actions = args.num_actions)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="full cooperation")
